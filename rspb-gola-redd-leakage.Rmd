---
title: 'VM0048: Leakage Belt Delineation of UDef LB'
author: "Winrock Intl"
date: "2025-02-26"
...
```{r, message=F, warning=F, error=F, comment=NA, eval=F, out.width="50%"}
...
```{r, echo=F, message=F, warning=F, error=F, comment=NA, eval=T, out.width="50%"}
...
tmap::tm_shape(slope_mask) + tmap::tm_borders(col="purple") +
  tmap::tm_shape(leakage_belt)) + tm_polygons(col="yellow",fill="yellow",fill_alpha=0.3)+ 
  tmap::tm_shape(aoi) + tmap::tm_borders(lwd=0.2, col="white") + 
  tmap::tm_text(text="Name", size=0.4, col="white") +
  tmap::tm_graticules(lines=T,labels.rot=c(0,90),lwd=0.2) +
  tmap::tm_scale_bar(position = c("RIGHT", "BOTTOM"), text.size = .5) + 
